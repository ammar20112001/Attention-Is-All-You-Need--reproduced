...
class PositionalEncodings(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        pe = [
            [
                math.sin(p / (10_000 ** ((2 * i) / self.d_model)))
                if i % 2 == 0
                else math.cos(p / (10_000 ** ((2 * i) / self.d_model)))
    ...
    def forward(self, x):
        ...
        x = x + (self.pe[:, : x.shape[1], :]).requires_grad_(
            False
        )  # This operation automatically broadcasts the Positional Encodings (pe) across all batches of x
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        # q/k --> (B, S, d_model) --> (B, S, h, dk) --> (B, h, S, dk)
        # v --> (B, S, d_model) --> (B, S, h, dv) --> (B, h, S, dv)
        query = query.view(query.shape[0], query.shape[1], self.h, self.dk).transpose(
            1, 2
        )
        key = key.view(key.shape[0], key.shape[1], self.h, self.dk).transpose(1, 2)
        value = value.view(value.shape[0], value.shape[1], self.h, self.dv).transpose(
            1, 2
        )
        ...
        x = (
            ...
            .contiguous()
            .view(attention.shape[0], -1, self.h * self.dv)
        )
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.residual_connection_block = nn.ModuleList(
...
def build_transformer(
    ...
    for _ in range(n_stack):
        ...
        encoder_blocks.append(
            EncoderBlock(
                d_model, multi_head_attention_block, feed_forward_block, dropout
            )
        )
